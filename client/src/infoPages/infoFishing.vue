...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchData = async () => {
      try {
        childList.value = await childService.getByName('admin/infopages', 'Fishing');
    ...
    const loadDataAndOpenDialog = async (id: string) => {
      try {
        const data: Info | Info[] = await childService.getAll('admin/infopages', { id, category: "Fishing" });
