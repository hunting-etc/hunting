  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const fetchData = async () => {
        try {
          childList.value = await childService.getByName('test/info-pages', 'News');
      ...
      const loadDataAndOpenDialog = async (id: string) => {
        try {
          const data: Child | Child[] = await childService.getAll('test/info-pages', { id, category: "News" });
  ...
  const onDialogHide = () => {
    window.editorInstance.clear();
      ...
      const deleteItem = async (id: string) => {
        ...
        if (confirmDelete) {
          try {
            await childService.delete('test/info-pages', id, '');
            fetchData();
          } catch (error) {
