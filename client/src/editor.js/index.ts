...
export default () => {
    ...
    document.querySelectorAll<HTMLElement>('.content-editor').forEach(($el) => {
        const input = $el.parentElement.querySelector<HTMLInputElement>('input[type="hidden"]');
        let data = null;
        ...
        const editor = new EditorJS({
            ...
            onChange() {
                editor.save().then((data) => input.value = JSON.stringify(data));
            },
            i18n: {
                messages: {
                    ui: {
