<template>
    <div class="panel">
      ...
      <div class="card flex justify-center">
        <InputText id="price" v-model.number="price" />
      </div>
      <Divider />
  ...
  const fetchData = async () => {
    ...
    photoUrl.value =`${infoService.baseUrl}${initialData.value.photo}` ; // Если это уже строка URL
  }
    ...
    globalError.value = ""; // Сбрасываем ошибку при успешной валидации
        try {        
          ...
          if (selectedCategory) {
            ...
            const selectedCategoryId=InfoList.value.find(category=> category.name===selectedCategory.value)?.id;
