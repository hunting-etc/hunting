...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchData = async () => {
      try {
        childList.value = await childService.getByName('admin/categories', 'Hunting');
    ...
    const loadDataAndOpenDialog = async (id: string) => {
      try {
        const data: Child | Child[] = await childService.getAll('admin/categories', { id, category: "Hunting" });
