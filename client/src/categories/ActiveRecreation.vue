<template>
  <div>
    ...
    <p>Информация о категориях активного отдыха.</p>
    <div class="header-container">
      ...
      <Dialog v-model:visible="createDialogVisible" modal header="Создание категории" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
        <Create @close="handleCreateDialogClose" category="ActiveRecreation" />
      </Dialog>
    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchData = async () => {
      try {
        childList.value = await childService.getByName('admin/categories', 'ActiveRecreation');
    ...
    const loadDataAndOpenDialog = async (id: string) => {
      try {
        const data: Child | Child[] = await childService.getAll('admin/categories', { id, category: "ActiveRecreation" });
